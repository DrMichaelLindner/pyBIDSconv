...
class GetInput(wx.Frame):
    def __init__(self):
        ...
        self.inputdir.SetValue('F:\DICOM_to_BIDS_test\DATA\BTPDLT\BTPDLT_01')
        self.subjnum.SetValue('4')
        self.bidsdir.SetValue("F:\DICOM_to_BIDS_test\\testbids\BTPDLT")
        self.cfgfile.SetValue('G:\Dropbox\Dropbox\pyBIDSconv\example_files\pyBIDSconv_CINNconfig.py')
        self.catfile.SetValue('G:\Dropbox\Dropbox\pyBIDSconv\example_files\pyBVIDSconv_CINNcatagorisation.txt')

        # self.inputdir.SetValue('/media/nn904630/BEAED8CEAED87FF7/DICOM_2_BIDS_test/DATA/BH_PhD_2/BH_PhD_2_004')
        # self.subjnum.SetValue('4')
        # self.bidsdir.SetValue('/media/nn904630/BEAED8CEAED87FF7/DICOM_2_BIDS_test/testbids')
        # self.cfgfile.SetValue('/home/nn904630/Dropbox/pyBIDSconv/example_files/pyBIDSconv_CINNconfig.py')
        # self.catfile.SetValue('/home/nn904630/Dropbox/pyBIDSconv/example_files/pyBVIDSconv_CINNcatagorisation.txt')

        ###############################################################################################################
        ###############################################################################################################

        # self.OKbutton = AB.AquaButton(panel, label = 'OK', pos = (20, 280+pp), size = (450, 70))
        # self.OKbutton.SetForegroundColour("black")
    ...
    def onbidshome(self, e):
        BIDShome()

    def onbidsspecs(self, e):
        BIDSspecs()

    def onvalidator(self, e):
        StartValidator()

    def onedit_ds(self, e):
        StartDSedit()

    def on_main_help(self, e):
        AboutMainHelp()
        ...
        try:
            for ii in range(len(un_seq_l)):
                if scantype_list[ii] == "fmap":
                    s = un_seq_l[ii]
                    for jj in range(len(cfg.PhaseInfoForFmapsByFilenameSubstring)):
                        if cfg.PhaseInfoForFmapsByFilenameSubstring[jj] in s.lower():
                            acq_name_list[ii] = cfg.PhaseInfoForFmapsByFilenameSubstring[jj][1:]
        except NotImplementedError:
            pass

        # define exclusions based on sequence name parts
        for ii in range(len(cfg.ExclusionsByFilenameContent)):
            c = [i for i, item in enumerate(un_seq_l) if cfg.ExclusionsByFilenameContent[ii] in item]
            for cc in range(len(c)):
                exclusion_array[c[cc]] = 1
                acq_name_list[c[cc]] = cfg.ExclusionsByFilenameContent[ii]

        # define exclusions based on sequence name end
        for ii in range(len(cfg.ExclusionsByFilenameEnd)):
            c = []
            for sco in range(len(un_seq_l)):
                if un_seq_l[sco].endswith(cfg.ExclusionsByFilenameEnd[ii]):
                    c.append(sco)
            #c = [i for i, item in enumerate(un_seq_l) if cfg.ExclusionsByFilenameEnd[ii] in item]
            for cc in range(len(c)):
                exclusion_array[c[cc]] = 1
                acq_name_list[c[cc]] = cfg.ExclusionsByFilenameEnd[ii]
...
class CheckSeqs(wx.Frame):
    def __init__(self, un_seq, scantype_list, exclusion_array, nrvols_array, subjectnumber, subjectgroup, sessionnumber, subjtext2log, acq_name_list, rec_name_list, label_list, dcmfiles, pathdicom, outputdir, it_list2, acq_time, patinfo, un_echo):
        ...
        for i in range(len(un_seq)):
            ...
            if scantype_list[i] == "func":
                ...
                # self.ref[i].SetSelection(refvalue[i])
            ...
            # self.line[i] = wx.StaticLine(self.panel, -1, wx.Point(150, self.vertshift+40*i-5), 
            #                              wx.Size(guiwidth-170, 7))
            # self.line[i].SetForegroundColour(wx.Colour(255, 0, 0))
    ...
    def oncombo1(self, event):
        ...
        app = wx.App()
        if selection == 0:
            # self.combo2[nr].SetForegroundColour(wx.BLACK)
            ...
            # self.label[nr].SetForegroundColour(wx.RED)
    ...
    def oncombo2(self, event):
        ...
        if selection == 2:
            ...
            # self.label[nr].SetSelection(reflab[nr])
            self.label[nr].Show()
    ...
    def oncheckbutton(self, event):
        ...
        # echo2conv = [x.encode('UTF8') for x in echo2conv]
        ...
        for ii in range(len(folder2conv)):
            ...
            for filename in onlyfiles:
                ...
                for ftype in filetypes:
                    ...
                    # dest = os.path.join(subjectfolder, folder2conv[ii], newfilename + ".nii" + ftype)
                    # scanstsv.append(os.path.join(subjectfolderrel, folder2conv[ii], newfilename + ".nii" + ftype))
                    # else:
